...
ABSL_FLAG(double, alpha, 1e-5, "ELO L2 regularization strength");
ABSL_FLAG(double, rater_prior_strength, 100.0,
          "Prior strength against random-answering raters");
...
void Run() {
  ...
  settings.alpha = static_cast<float>(absl::GetFlag(FLAGS_alpha));
  settings.rater_prior_strength =
      static_cast<float>(absl::GetFlag(FLAGS_rater_prior_strength));
  CHECK_GT(settings.alpha, 0.0);
  CHECK_GT(settings.rater_prior_strength, 0.0);
  ELOData elo_data(settings, 1, methods);
  for (const auto& ans : answers) {
